...
business_df['geohash'] = business_df.apply(lambda x: geohash.encode(x['latitude'], x['longitude'], precision=5), axis=1)
...
def process_review(review_df):
    # Preprocess review data
    threshold_dict = {
        'positive': 5,  #stars
        'negative': 2,  #stars
        'hard_negative_ratio': 0.9,    #stars_ratio
        'hard_negative_sentiment': -0.2,    #sentiment_score
        'hard_positive_ratio': 1.05,    #stars_ratio
        'hard_positive_sentiment': 0.98,    #sentiment_score
    }
    # Work on a copy to avoid modifying the original df if needed
    df = review_df.copy()
    df['difficulty'] = 0
    
    # Initialize cumulative DataFrames for positive and negative reviews.
    positive_df = pd.DataFrame(columns=df.columns)
    negative_df = pd.DataFrame(columns=df.columns)
    
    # Step 1: Select easy positives and negatives based on stars threshold.
    easy_positive_df = df[df['stars'] >= threshold_dict['positive']]
    easy_positive_df['label'] = 1
    df = df.drop(easy_positive_df.index)  # remove these from df
    positive_df = pd.concat([positive_df, easy_positive_df])
    
    easy_negative_df = df[df['stars'] <= threshold_dict['negative']]
    easy_negative_df['label'] = 0
    df = df.drop(easy_negative_df.index)  # remove these from df
    negative_df = pd.concat([negative_df, easy_negative_df])
    
    # Step 2: Further add hard positives from remaining records.
    hard_positive_df = df[
        (df['stars_ratio'] >= threshold_dict['hard_positive_ratio']) &
        (df['sentiment_score'] >= threshold_dict['hard_positive_sentiment'])
    ]
    hard_positive_df['label'] = 1
    hard_positive_df['difficulty'] = 1
    df = df.drop(hard_positive_df.index)  # remove these from df
    positive_df = pd.concat([positive_df, hard_positive_df])
    
    # Step 3: Further add hard negatives from remaining records.
    hard_negative_df = df[
        (df['stars_ratio'] <= threshold_dict['hard_negative_ratio']) &
        (df['sentiment_score'] <= threshold_dict['hard_negative_sentiment'])
    ]
    hard_negative_df['label'] = 0
    hard_negative_df['difficulty'] = 1
    df = df.drop(hard_negative_df.index)  # remove these from df
    negative_df = pd.concat([negative_df, hard_negative_df])

    labeled_df = pd.concat([positive_df, negative_df])
    return labeled_df
...
def prepare_data(user_df, business_df, review_df, categories_df, user_id_encoder, business_id_encoder, categories_encoder, business_geohash_encoder, user_scaler, business_scaler, use_stage='train', default_user_id='default_user', default_business_id='default_business'):
    ...
    review_df['stars_ratio'] = review_df['stars']/review_df['average_stars']
    label_df = process_review(review_df)
    ...
    return user_df, business_df, review_df, label_df, user_continuous_features_scaled, business_continuous_features_scaled, num_users, num_businesses, num_categories, num_geohashes
...
def load_saved_models(save_folder_path='Saved_Triplet_Hinge_Loss/'):
    ...
    with open(save_folder_path + 'business_geohash_encoder.pkl', 'rb') as f:
        business_geohash_encoder = pickle.load(f)
    # business_geohash_encoder = None
    ...
    return user_model, item_model, user_id_encoder, business_id_encoder, categories_encoder, business_geohash_encoder, user_scaler, business_scaler, 
...
def generate_triplets(review_df, num_neg_samples=1):
    ...
    for user_id, group in grouped:
        ...
        for _, neg_row in negative_samples.iterrows():
            for _, pos_row in positive_samples.sample(num_neg_samples, replace=True).iterrows():
                triplets.append((
                    user_id,

...
<h1>5/7/2024: Red Content Recommendation System Tutorial (Updated on 17/7/2024) </h1>
...
4. **Re-Ranking** (reduce results from hundreds to tens)
   - It considers both scores and the variety of the results, setting up rules to redistribute similar results.
...
- **User-User Index:**
   ...
   Stores user similarity. Then, it can be used to determine if two users are similar.
...
- **Misclassification of Similar Users:**
   ...
   We need to lower the weighting of popular items to reduce their effect on user similarity.

### Discrete Features
This section will mention how we handle different discrete features (e.g., country, item_id, etc.). Discrete features are discontinuous and have definite feature boundaries (i.e., fixed values).
